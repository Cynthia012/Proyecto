...
export class AdminComponent implements OnInit {
                  ...
                  this.ngZone.run(() => {
                    if (!user) {
                      ...
                      if (user.displayName=== 'marksuckerberg69') {
                        this.flag = true;
                        this.mail.getQuejas().subscribe((data)=>{
                          if(JSON.stringify(data).length >0){
                              this.quejas_flag = true;
                              this.quejas = data;
                          }
                        });
                        this.post.getAllPosts().subscribe((data) => {
                          this.data_categories = data['resp1'];
                          this.growing = data['resp2'];
  ...
  eliminarQuejas(){
    this.quejas_flag = false;
    this.quejas = null;
    this.mail.delteQuejas().subscribe((data) => {console.log(data)});
  }
  mostrar() {
    ...
    if (!this.grow_flag) {
      ...
      for (let i = 0; i < node._groups[0].length - 1; i++) {
        ...
        svg
          ...
          .attr('x2', (d) => node._groups[0][i].getAttribute('cx'))
          .attr('y1', (d) => node._groups[0][i].getAttribute('cy'))
          .attr('y2', (d) => node._groups[0][i].getAttribute('cy'))
          .transition()
          .duration(500)
